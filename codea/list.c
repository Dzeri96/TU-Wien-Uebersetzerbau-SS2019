...
list_t* mergeLists(list_t* list1, list_t* list2, short errorOnDuplicate){
	...
	list_t* cur2 = list2;
	while(cur2 != NULL){
		newlist = addToList(newlist, cur2->name, cur2->label, errorOnDuplicate, cur2->parIndex);
		cur2 = cur2->next;
	}
...
list_t* addToList(list_t* list, char* name, short label, short errorOnDuplicate, short parIndex){
...
list_t* duplicateList(list_t* list){
	...
	if(list == NULL)
		return newlist;
	while(cur != NULL){
		newlist = addToList(newlist, cur->name, cur->label, TRUE, cur->parIndex);
		cur = cur->next;
	}
	return newlist;
}
