...
char *parRegisters[]={"rdi", "rsi", "rdx", "rcx", "r8", "r9"};
...
void saveRegsUntil(char *reg){
	short i;
	for (i = 0; i<9; i++){
		if(!strcmp(reg, registers[i])) break;
		printf("\tpush %%%s\n", registers[i]);
	}
}
...
void saveParRegsUntil(short nr){
	short i;
	for (i = 0; i<nr; i++){
		printf("\tpush %%%s\n", parRegisters[i]);
	}
...
void restoreRegsUntilExcept(char *until, char *except){
	short i;
	short popIt = 0;
	for(i = 8; i>-1; i--){
		if(!strcmp(until, registers[i])){
			popIt = 1;
			continue;
		}
		if(strcmp(registers[i], except) && popIt){
			printf("\tpop %%%s\n", registers[i]);
		}
	}
}

void restoreParRegsUntil(short nr){
	short i;
	for(i = nr-1; i>-1; i--){
		printf("\tpop %%%s\n", parRegisters[i]);
	}
...
void restoreParRegsUntilRev(short nr){
	short i;
	for(i = 0; i<nr ; i++){
		printf("\tpop %%%s\n", parRegisters[i]);
	}
...
void restoreRegExcept(char *reg){
	short i;
	for(i = 8; i>-1; i--){
