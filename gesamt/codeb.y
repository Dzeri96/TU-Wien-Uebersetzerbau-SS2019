...
@attributes { list_t *ids; tree_t *node; } Expr Term ExprStartRep PlusTermRep MulTermRep OrTermRep DotTermRep MaybeExpr MaybeID
...
@attributes { list_t *ids; tree_t *node; short parCnt; } ExprRep ExprRepRep
...
Term:		'(' Expr ')'
	...
	@}
	|	ID '(' ')'
	@{	@i @Term.node@ = newNode(OP_Call, NULL, NULL);
		@codegen @revorder(1) saveRegsUntil(@Term.node@->reg); saveParRegsUntil(getNrPar(@Term.ids@));
		@codegen call(@ID.name@); move("rax", @Term.node@->reg); restoreParRegsUntil(getNrPar(@Term.ids@)); restoreRegsUntilExcept(@Term.node@->reg,"none"); 	
	 @}
	|	ID '(' ExprRep ')'
	@{	@i @Term.node@ = newNode(OP_Call, NULL, NULL); 
		@codegen @revorder(1) saveRegsUntil(@Term.node@->reg); saveParRegsUntil(getNrPar(@Term.ids@));
		@codegen burm_label(@ExprRep.node@); burm_reduce(@ExprRep.node@,1);
		@codegen restoreParRegsUntil(@ExprRep.parCnt@); call(@ID.name@); move("rax", @Term.node@->reg); restoreParRegsUntil(getNrPar(@Term.ids@)); restoreRegsUntilExcept(@Term.node@->reg,"none");

	@}
	;

ExprRep:	ExprRepRep Expr
	@{	@i @ExprRep.node@ = newNode(OP_Expr, @Expr.node@, NULL); 
		@i @ExprRep.parCnt@ = @ExprRepRep.parCnt@ + 1;
		@reg @Expr.node@->reg = getRegByNr(getNrDefVar(@ExprRep.ids@));
	@}
	;

ExprRepRep:	
	@{	@i @ExprRepRep.node@ = newNode(OP_Blank, NULL, NULL); 
		@i @ExprRepRep.parCnt@ = 0;
	@}
	|	ExprRepRep Expr ','
	@{	@i @ExprRepRep.0.node@ = newNode(OP_Expr, @Expr.node@, NULL);  
		@i @ExprRepRep.0.parCnt@ = @ExprRepRep.1.parCnt@ + 1;
		@reg @Expr.node@->reg = getRegByNr(getNrDefVar(@ExprRepRep.0.ids@)); 
		@codegen burm_label(@ExprRepRep.node@); burm_reduce(@ExprRepRep.node@,1);
	@}
	;
%%
int main (void) {
